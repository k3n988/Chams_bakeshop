...
class SellerMonthlyScreen extends StatefulWidget {
  ...
  @override
  State<SellerMonthlyScreen> createState() => _SellerMonthlyScreenState();
}
        ...
        child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
          ...
          if (vm.isLoading)
            ...
            GridView.count(
              ...
              children: [
                _GridStat(icon: Icons.sell_outlined,         label: 'Total Sold',
                    value: '${vm.totalPiecesSold} pcs',      color: AppColors.success),
                _GridStat(icon: Icons.undo_outlined,          label: 'Total Returns',
                    value: '${vm.totalReturns} pcs',          color: AppColors.warning),
                _GridStat(icon: Icons.payments_outlined,      label: 'Remitted',
                    value: formatCurrency(vm.totalActualRemittance), color: AppColors.seller),
                _GridStat(icon: Icons.calendar_today_outlined, label: 'Days Active',
                    value: '${vm.daysRemitted} days',         color: const Color(0xFF1976D2)),
              ],
            ),

            // ── Monthly salary banner ────────────────────
            _MonthlySalaryBanner(vm: vm),
  ...
  Widget build(BuildContext context) => Container(
        ...
        decoration: BoxDecoration(
          gradient: LinearGradient(
            colors: [AppColors.seller, AppColors.seller.withValues(alpha: 0.70)],
        ...
        child: Column(children: [
          ...
          Text(formatCurrency(vm.totalActualRemittance),
          ...
          Container(
            ...
            child: Row(mainAxisSize: MainAxisSize.min, children: [
              const Icon(Icons.sell_outlined, size: 13, color: Colors.white70),
              const SizedBox(width: 6),
              Text('${vm.totalPiecesSold} pieces sold',
                  style: const TextStyle(
                      fontSize: 12, color: Colors.white, fontWeight: FontWeight.w600)),
              const SizedBox(width: 14),
              const Icon(Icons.calendar_view_week_outlined, size: 13, color: Colors.white70),
              const SizedBox(width: 6),
              const Text('4 weeks',
                  style: TextStyle(
                      fontSize: 12, color: Colors.white, fontWeight: FontWeight.w600)),
...
// ── Monthly salary banner — reads from remittances directly ───
class _MonthlySalaryBanner extends StatelessWidget {
  final SellerRemittanceViewModel vm;
  const _MonthlySalaryBanner({required this.vm});

  @override
  Widget build(BuildContext context) {
    // Sum salary from all remittances — no dependency on _WeeklySummary
    final totalSalary = vm.sortedRemittances.fold(0.0, (s, r) => s + r.salary);
    if (totalSalary <= 0) return const SizedBox.shrink();
          ...
          const Row(children: [
            ...
            Text('TOTAL MONTHLY SALARY',
                style: TextStyle(fontSize: 11, fontWeight: FontWeight.w800,
                    color: AppColors.seller, letterSpacing: 0.5)),
          ]),
          const SizedBox(height: 4),
          const Text('Sum of all session salaries',
              style: TextStyle(fontSize: 11, color: AppColors.textHint)),
        ]),
        Text(formatCurrency(totalSalary),
            style: const TextStyle(
                fontSize: 22, fontWeight: FontWeight.w900, color: AppColors.seller)),
      ]),
    );
  }
}
...
// ── Week summary row — NO totalSalary reference ───────────────
class _WeekSummaryRow extends StatelessWidget {
  final int     index;
  final dynamic summary; // _WeeklySummary — only use: weekStart, weekEnd, days, piecesSold, totalRemittance
  final bool    isLast;
  const _WeekSummaryRow({
    required this.index, required this.summary, required this.isLast,
  });
  ...
  Widget build(BuildContext context) => Container(
        ...
        child: Row(children: [
          Container(
            ...
            child: Text('W${index + 1}',
                style: TextStyle(
                    ...
                    color: isLast ? AppColors.seller : AppColors.textHint)),
          ),
          ...
          Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
            ...
            if (summary.days > 0)
              Text(
                '${summary.days} days · ${summary.piecesSold} pcs sold',
          ...
          Text(
            ...
            style: TextStyle(
                ...
                color: summary.totalRemittance > 0
                    ? AppColors.primaryDark : AppColors.textHint),
          ),
        ]),
      );
}
